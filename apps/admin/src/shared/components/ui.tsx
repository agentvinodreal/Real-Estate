...
/* ── Generic modal (popup overlay for forms, etc.) ──────────────────── */

/** Fixed-position backdrop + centered panel. Closes on backdrop click. */
export function Modal({ onClose, className = '', children }: { onClose: () => void; className?: string; children: ReactNode }) {
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-ink/50 p-4 py-10 sm:items-center"
      onClick={onClose}
    >
      <motion.div
        ...
        transition={{ duration: 0.3, ease: EASE_OUT_EXPO }}
        className={`w-full max-w-2xl border border-ink/15 bg-bone shadow-xl ${className}`}
